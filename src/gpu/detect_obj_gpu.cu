...
void cpyCudaToCuda(unsigned char* src,
                   unsigned char* dst,
                   size_t rows,
                   size_t cols,
                   size_t pitch)
{
  cudaMemcpy2D(dst, pitch, src, pitch, cols * sizeof(unsigned char), rows,
...
void to_save(unsigned char* buffer_cuda,
             int rows,
             int cols,
             std::string file,
             size_t pitch)
                               ...
                               int* nb_obj)
{
  ...
  unsigned char* img_obj_cuda =
      cpy_host_to_device<unsigned char>(img_obj, size_color);

  gray_scale_gpu(img_obj_cuda, buffer_obj_cuda, rows, cols, pitch, channels,
                 thx, thy);
  apply_blurr_gpu(buffer_obj_cuda, rows, cols, gaussian_k_size, gaussian_k,
                  pitch, thx, thy);
  difference_gpu(blur_ref_cuda, buffer_obj_cuda, rows, cols, pitch, thx, thy);

  unsigned char* current_obj = buffer_obj_cuda;

  erosion_gpu(current_obj, rows, cols, morpho_k1_size, morpho_k1, pitch, thx,
              thy);
  dilation_gpu(current_obj, rows, cols, morpho_k1_size, morpho_k1, pitch, thx,
               thy);

  dilation_gpu(current_obj, rows, cols, morpho_k2_size, morpho_k2, pitch, thx,
               thy);
  erosion_gpu(current_obj, rows, cols, morpho_k2_size, morpho_k2, pitch, thx,
              thy);

  unsigned char otsu_th = threshold(current_obj, rows, cols, pitch, thx, thy);
  int nb_components =
      connexe_components(current_obj, rows, cols, pitch, otsu_th, thx, thy);
                                  ...
                                  int* nb_objs)
{
  ...
  unsigned char* buffer_ref_cuda =
      cpy_host_to_device<unsigned char>(images[0], size_color);
  ...
  gray_scale_gpu(buffer_ref_cuda, gray_ref_cuda, rows, cols, pitch, channels,
                 threads.x, threads.y);
  ...
  apply_blurr_gpu(gray_ref_cuda, rows, cols, kernel_size, kernel_gpu, pitch,
                  threads.x, threads.y);
  ...
  for (int i = 1; i < length; i++) {
    all_boxes[i - 1] = object_detection(
        gray_ref_cuda, images[i], gray_obj_cuda, rows, cols, pitch, channels,
        kernel_gpu, kernel_size, morpho_k1, k1_size, morpho_k2, k2_size,
        threads.x, threads.y, &nb_obj);
    nb_objs[i - 1] = nb_obj;
  }
                ...
                int channels)
{
  ...
  gray_scale_gpu(buffer_obj_cuda, gray_obj_cuda, rows, cols, pitch, channels,
                 threadsPerBlock.x, threadsPerBlock.y);
  ...
  apply_blurr_gpu(gray_ref_cuda, rows, cols, kernel_size, kernel_gpu, pitch,
                  threadsPerBlock.x, threadsPerBlock.y);
  apply_blurr_gpu(gray_obj_cuda, rows, cols, kernel_size, kernel_gpu, pitch,
                  threadsPerBlock.x, threadsPerBlock.y);
  ...
  // Calculating diff
  difference_gpu(gray_ref_cuda, gray_obj_cuda, rows, cols, pitch,
                 threadsPerBlock.x, threadsPerBlock.y);
  ...
  // Perform closing
  erosion_gpu(current_obj, rows, cols, k1_size, morpho_k1, pitch,
              threadsPerBlock.x, threadsPerBlock.y);
  dilation_gpu(current_obj, rows, cols, k1_size, morpho_k1, pitch,
               threadsPerBlock.x, threadsPerBlock.y);

  to_save(current_obj, rows, cols, file_save_closing_obj, pitch);

  // Perform opening
  dilation_gpu(current_obj, rows, cols, k2_size, morpho_k2, pitch,
               threadsPerBlock.x, threadsPerBlock.y);
  erosion_gpu(current_obj, rows, cols, k2_size, morpho_k2, pitch,
              threadsPerBlock.x, threadsPerBlock.y);
  ...
  unsigned char otsu_th = threshold(current_obj, rows, cols, pitch,
                                    threadsPerBlock.x, threadsPerBlock.y);
  to_save(current_obj, rows, cols, file_save_threshold_obj, pitch);

  int nb_components =
      connexe_components(current_obj, rows, cols, pitch, otsu_th,
                         threadsPerBlock.x, threadsPerBlock.y);
